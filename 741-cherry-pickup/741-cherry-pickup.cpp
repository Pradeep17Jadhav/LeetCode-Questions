...
public:
    int cherryPickup(vector<vector<int>>& grid) {
        ...
        int cherries = memoization(grid, dp, 0, 0, 0);
        return cherries <= 0 ? 0 : cherries;
    }
    
    int memoization(vector<vector<int>> &grid, vector<vector<vector<int>>> &dp, int row1, int col1, int col2) {
        int n = grid.size();
        ...
        int maxCherries = -1e9;
        int downdown = memoization(grid, dp, row1+1, col1, col2);
        int downright = memoization(grid, dp, row1+1, col1, col2+1);
        int rightdown = memoization(grid, dp, row1, col1+1, col2); //increasing right will automatically take care of increasing row
        int rightright = memoization(grid, dp, row1, col1+1, col2+1);
